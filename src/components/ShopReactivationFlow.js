...
export const ShopReactivationFlow = ({ shopId }) => {
  ...
  const checkStatus = async () => {
    try {
      const response = await fetch(`http://192.168.0.132:3000/api/shops/${shopId}/status`);
  ...
  const requestReactivation = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`http://192.168.0.132:3000/api/shops/${shopId}/reactivate`, {
        method: 'POST'
      });
